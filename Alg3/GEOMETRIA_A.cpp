...
float area_poligono(list<ponto> pontos)
{
    float area = 0;
    int n = pontos.size();
    ponto v[99];
...
int main()
{
    double resp = 0;
    int reservas, x;
    cout << "QUANTAS RESERVAS: ";
    cin >> reservas;

    for (x = 0; x < reservas; x++)
    {
        int n;
        ponto v[99];
        ...
        list<ponto> fecho_convexo = Graham_scan(v, n);
        resp += area_poligono(fecho_convexo);
    }
